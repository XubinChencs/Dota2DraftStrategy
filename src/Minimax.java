...
public class Minimax {
	...
	private double minimax(State state, int round, boolean isTeam1) {
		// leaf node
		if (round == 7) {
			  return state.diff();
		}
		...
		if (isTeam1) {
			...
			for (String banned : curHeroPool) {    // ban 1 hero
				...
				if (round == 1) {
					value = Math.max(value, minimax(state, round, false));
				} else {
			...
			for (int i = 0; i < curHeroPool.size(); i++) { // pick first hero
				...
				if (round == 6) {
					value = Math.min(value,  minimax(state, round + 1, true));
				} else {
	...
	public String[] nextStep(State state, int round) {
		...
		for (String banned : curHeroPool) {
			...
			if (round == 1) {
				double newValue = minimax(state, round, false);
				if (value < newValue) {
					value = newValue;
					banPick[0] = banned;
				}
				...
				for (String picked : curHeroPool) {
					if (!picked.equals(banned)) {
						state.update(true, true, picked);
						double newValue = Math.max(value,  minimax(state, round, false));
						if (value < newValue) {
							value = newValue;
							banPick[0] = banned;
							banPick[1] = picked;
